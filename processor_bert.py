...
def classify_with_bert(log_message):
    ...
    # The classification model we create is a logistic regression
    # We can get probabilities -- predict_proba)(embeddings)
    probabilities = model_classification.predict_proba(embeddings)[0]
    if max(probabilities) < 0.5: # we get the max value of the probabilities
        return "Unclassified" # Return unclassified since, probabilites of less than .5 means its really not sure
